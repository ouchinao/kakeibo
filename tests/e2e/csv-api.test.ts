...
describe("CSV import/export API", () => {
  test("exports recorded transactions as a CSV attachment", async () => {
    ...
    expect(text).toContain("EXPENSE,NEEDS,1500,JPY,1500,JPY,Groceries");
  });
  ...
  test("export then re-import round-trips the data", async () => {
    ...
    try {
      ...
      const list = (await (
        ...
        baseAmount: { formatted: string };
        note: string;
      }>;
      expect(list[0]?.amount.formatted).toBe("$12.34");
      // The base-currency amount must survive the round-trip, not reset to USD.
      expect(list[0]?.baseAmount.formatted).toBe("¥1,851");
      expect(list[0]?.note).toBe("Coffee, large");

      // ...and it must still aggregate into the base-currency monthly total.
      const summary = (await (
        await fresh.fetch(new Request("http://test.local/api/summary?month=2026-05"))
      ).json()) as { currency: string; totalExpense: { minor: number } };
  ...
  test("rejects importing a foreign-currency row without a base amount", async () => {
    const csv = [
      "date,type,category,amount,currency,note",
